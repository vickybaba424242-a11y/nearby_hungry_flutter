...
class _RegisterPageState extends State<RegisterPage> {
  ...
  Future<String?> getOldReferralCode(
      String email,
      ) async {

    final deletedUser = await _firestore
        .collection('deleted_users')
        .doc(email.toLowerCase())
        .get();

    if (deletedUser.exists) {
      return deletedUser.data()?['referralCode'];
    }
  ...
  Future<void> _registerUser() async {
    ...
    try {
      ...
      final oldCode =
      await getOldReferralCode(email);

      if (oldCode != null &&
          enteredReferralCode.isNotEmpty) {

        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Referral code can only be used by new users',
            ),
          ),
        );
      ...
      if (enteredReferralCode.isNotEmpty) {
        ...
        final referralCheck = await _firestore
            .collection('users')
            .where(
          'referralCode',
          isEqualTo: enteredReferralCode,
        )
            .limit(1)
            .get();
      ...
      if (user != null) {
        ...
        String? oldCode = await getOldReferralCode(email);

        String myReferralCode =
            oldCode ?? await createUniqueReferralCode();
        ...
        final userData = {
          ...
          'referredBy': enteredReferralCode,
        };
